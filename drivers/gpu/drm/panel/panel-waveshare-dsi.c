...
static int ws_panel_disable(struct drm_panel *panel)
{
	...
	rockchip_panel_notifier_call_chain(&ts->panel_notifier,
...
ws_panel_create_backlight(struct ws_panel *ts)
{
	...
	return backlight_device_register(dev_name(dev), dev, ts,
...
/*        rockchip_panel_notifier_register(&ts->base,
                                              &ts->panel_notifier);
*/
...
//	 drm_panel_of_backlight(&ts->base);
...
static void ws_panel_remove(struct i2c_client *i2c)
{
	...
	ws_panel_disable(&ts->base);
        ...
        err = mipi_dsi_detach(ts->dsi);
        if (err < 0)
                dev_err(&ts->i2c->dev, "failed to detach from DSI host: %d\n", err);
...
static struct mipi_dsi_driver ws_panel_dsi_driver = {
        .driver.name = WS_DSI_DRIVER_NAME,
...
static int __init ws_panel_init(void)
{
        mipi_dsi_driver_register(&ws_panel_dsi_driver);
        return i2c_add_driver(&ws_panel_driver);
}
module_init(ws_panel_init);

static void __exit ws_panel_exit(void)
{
        i2c_del_driver(&ws_panel_driver);
        mipi_dsi_driver_unregister(&ws_panel_dsi_driver);
}
module_exit(ws_panel_exit);
